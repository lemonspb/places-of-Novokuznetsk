...
const Header = () => {
  ...
  const content = (
    <div>
      ...
      Нажимай на памятное место на карте и оставляй свои
      записки.</p>
  ...
  const menu = (
    <Menu className='header__menu'>
      ...
      <Menu.Item key="1">
        {currentUser 
          ? <Link to='/edit-account' >Редактировать</Link> 
          : <Link to='/signup'>Зарегистрироваться</Link>}

      </Menu.Item>
      <Menu.Item key="2">
        {currentUser 
          ? <Link to='/' onClick={() => { firebases.auth().signOut() }}> Выйти</Link> 
    ...
    <div className='header'>
      ...
      <Dropdown overlay={menu} className='header__dropdown'>
        <div className='header__avatar-wrap'>
          ...
          <div className='header__diplayname'>{currentUser?.displayName 
